...
namespace metapod
{
  ...
  struct jac_forward_propagation
  {
    ...
    static void run()
    {
      ...
      Robot::J.template block<6, Node::Joint::NBDOF>(
              6*Node::Body::label, Node::Joint::positionInConf) =
          Node::Joint::S;

      // contribution of the ancestor joints to the current body velocity,
      // copied from the parent body jacobian.
      // We pass CurrentBody::Joint::sXp.toMatrix() to avoid toMatrix() to
      // be called again at each step.
      jac_backward_propagation< Robot, CurrentBody, typename CurrentBody::Parent, typename CurrentBody::Parent >::run(CurrentBody::Joint::sXp.toMatrix());
  ...
  struct jac_backward_propagation
  {
    static void run(const matrix6d & sXp_matrix)
    {

      // contribution of the ancestor joint to the current body velocity,
      // copied from the parent body jacobian
      Robot::J.template block<6, AncestorBody::Joint::NBDOF>(
              6*CurrentBody::label, AncestorBody::Joint::positionInConf) =
        sXp_matrix
        * Robot::J.template block<6, AncestorBody::Joint::NBDOF>(
                6*ParentBody::label, AncestorBody::Joint::positionInConf);
      jac_backward_propagation< Robot,
                                CurrentBody,
                                ParentBody,
                                typename AncestorBody::Parent >::run(sXp_matrix);
    }
  };
  ...
  {
    static void run(const matrix6d &) {}
  };
